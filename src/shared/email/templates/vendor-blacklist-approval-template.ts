 ...
 *    - vendorActivatedSubject(data)           → subject line (plain text)
 *    - vendorActivatedTemplate(data)          → "vendor approved & now ACTIVE" email (creator + approver)
 *
...
/* ───────────────────────── 6e. Template: vendor approved & activated ───────────────────────── */

export type ActivationAudience = 'creator' | 'approver' | 'all';

export interface VendorActivatedData {
  vendorName: string;
  vendorCode: string;
  /** New status, e.g. "ACTIVE". */
  vendorStatus: string;
  /** Status before approval. Defaults to "UNDER_EVALUATION". */
  previousStatus?: string;
  /** Display name of the vendor type (resolve vendorTypeId → name before calling). */
  vendorType: string;
  approvedBy: string;
  approvedOn: string | Date;

  /* Optional extras (rows are hidden when empty) */
  tradeName?: string;
  primaryContactPerson?: string;
  /** Vendor's contact email. When empty, the email shows a "missing contact email" note. */
  contactEmail?: string;
  createdBy?: string;
  createdOn?: string | Date;
  /** Approver's comments. */
  decisionComments?: string;
  /** Defaults to "all" (one email to creator, cc approver). */
  audience?: ActivationAudience;
  /** Link to the vendor record. Adds a "View Vendor" button. */
  vendorUrl?: string;
  supportEmail?: string;
}
...
/** Plain-text subject line (do not HTML-escape). */
export const vendorActivatedSubject = (d: VendorActivatedData): string =>
  `Vendor activated: ${d.vendorCode} — ${d.vendorName}`;

export const vendorActivatedTemplate = (d: VendorActivatedData): string => {
  const audience = d.audience ?? 'all';
  const vendor = `${escapeHtml(d.vendorCode)} — ${escapeHtml(d.vendorName)}`;
  const status = escapeHtml(humanizeStatus(d.vendorStatus || 'ACTIVE'));
  const previous = escapeHtml(humanizeStatus(d.previousStatus || 'UNDER_EVALUATION'));
  const approvedOn = formatDate(d.approvedOn);
  const approver = d.approvedBy.includes('@') ? mailto(d.approvedBy) : escapeHtml(d.approvedBy);
  const approverText = escapeHtml(d.approvedBy);
  const b = (html: string) => `<strong style="${st('strong')}">${html}</strong>`;
  const opt = (label: string, value: string | undefined, render: (v: string) => string = escapeHtml) =>
    value && String(value).trim() ? [{ label, value: render(String(value).trim()) }] : [];

  const intro: Record<ActivationAudience, string> = {
    creator: `Vendor ${b(vendor)}, which you created, has been ${b('approved')} by ${approverText} and is now ${b(status)}.`,
    approver: `This confirms that you ${b('approved')} vendor ${b(vendor)}. Its status is now ${b(status)}.`,
    all: `Vendor ${b(vendor)} has been ${b('approved')} by ${approverText} and is now ${b(status)}.`,
  };

  const statusChange =
    `<span class="em-surface em-muted" style="${st('badgeNeutral')}">${previous}</span>` +
    `&nbsp;&nbsp;<span class="em-muted" style="${css({ color: c.textSubtle })}">→</span>&nbsp;&nbsp;` +
    `<span class="em-success em-success-surface" style="${st('badgeSuccess')}">${status}</span>`;

  const vendorRows = [
    { label: 'Vendor Code', value: escapeHtml(d.vendorCode) },
    { label: 'Vendor Name', value: escapeHtml(d.vendorName) },
    ...opt('Trade Name', d.tradeName),
    { label: 'Vendor Type', value: escapeHtml(d.vendorType || '—') },
    { label: 'Vendor Status', value: `<span class="em-success em-success-surface" style="${st('badgeSuccess')}">${status}</span>` },
    ...opt('Primary Contact', d.primaryContactPerson),
    ...opt('Contact Email', d.contactEmail, mailto),
    ...opt('Created By', d.createdBy, (v) => (v.includes('@') ? mailto(v) : escapeHtml(v))),
    ...(d.createdOn ? [{ label: 'Created On', value: formatDate(d.createdOn) }] : []),
  ];
  ...
  const missingEmail = d.contactEmail && d.contactEmail.trim()
    ? ''
    : callout(
        'danger',
        '📭',
        'No contact email on file.',
        'Add a contact email to the vendor profile so RFQs and purchase orders can be sent to the vendor.',
        true,
      );
  ...
  const body = `
              ...
              ${detailsCard('Vendor details', vendorRows)}

              ${detailsCard('Approval', approvalRows)}

              ${impactList(
                'The vendor is now available for',
                ['Vendor selection', 'RFQs', 'New purchase orders'],
                'Review the vendor profile to confirm contact, tax and bank details are complete.',
                'ok',
              )}
              ...
              ${infoPills(
                { icon: '✅', label: 'Status', value: status, tone: 'primary' },
                { icon: '📅', label: 'Active since', value: approvedOn, tone: 'secondary' },
              )}

              ${missingEmail}
              ${audience === 'approver' ? '' : callout('info', 'ℹ️', 'Think this is a mistake?', 'Contact your procurement administrator.', !missingEmail)}`;

  return renderShell({
    title: vendorActivatedSubject(d),
    preheader: `${d.vendorCode} — ${d.vendorName} was approved by ${d.approvedBy} and is now ${humanizeStatus(d.vendorStatus || 'ACTIVE')}.`,
    heroIcon: THEME.heroIconApproved,
    heroIconAlt: 'Approved',
    heroTitle: 'Vendor Approved & Activated',
    heroSubtitle: `${d.vendorName} is now ${humanizeStatus(d.vendorStatus || 'ACTIVE').toLowerCase()}.`,
    body,
    supportEmail: d.supportEmail,
  });
};
